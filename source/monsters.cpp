...
class monster {
  // just for ease of reading. Members are initialised as private by default if
  // no access specifier is used
private:
  // longest base name is 12 letters, ie; size = 13, prefix "Mega " = 5 & suffix
  // " X" = 3
  char mname[21];
...
public:
  ...
  unsigned int getattack() { return (atk); }
  unsigned int getdefence() { return (def); }
  unsigned int getspeed() { return (speed); }
  TYPE gettype1() { return (t1); }
  TYPE gettype2() { return (t2); }
  ...
  int getpre_dmg(attack move) {
    // based on level 50, as it's the official level cap. Formulae Sourced from
    // serebii.net/games/damage.shtml
  ...
  int statcalc(unsigned int b[2], unsigned int a[2], unsigned int d[2],
               unsigned int s[2]) {
    // for base HP
...
// consider possible convertion to class from struct in future for storing
// player stats
struct player {
  char id[30];
  char password[30];
...
monster rec_mon() {
  ...
  do {
    ...
    choice =
        'x'; // placeholder to be replaced by a proper input through interface
  } while (choice != 'x');
  fi.close();
  return (ch);
}
