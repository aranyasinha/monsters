...
void dialogue(int no, char Pname[20]) {
  ...
  int r, len;
  char a[500];
  initgraph(&driver, &gmd, "C:\\TURBOC3\\BGI");
  ...
  if (no == 4) {
    ...
    strcpy(a, "So you've come to face the elite 4! You are brave, I give you "
              "that, but also foolish. I am the first elite 4 member, "
              "Lorelette. To move forward, you have to go through me. Be ready "
              "to face the fury of psychic types!");
    cout << "Lorelette : ";
    len = strlen(a);
    for (int i = 0; i < len; i++) {
      delay(30);
    ...
    strcpy(a, "Ahoy! I am Bruno.....2nd elite 4 member. I am one of the "
              "toughest trainers of all time. With the power of normal types, "
    ...
    strcpy(a, "I am The Master of Dragons.....Lance! With all my abilities as "
              "a Dragon Trainer combined with the Mystical Powers of The "
              "Dragons. I become UNDEFEATABLE!");
    cout << "Lance : ";
    len = strlen(a);
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (a[i] == ',' || a[i] == '.' || a[i] == '!') {
        /*if (i==167-16)
        {
          ...
          setcolor(9);
          settextstyle(TRIPLEX_FONT, HORIZ_DIR,2);
          outtextxy((getmaxx()/2)-(textwidth(msg)/2),(getmaxy()/2)-(textheight(msg)/2),
        msg); getch(); closegraph(); clrscr();
        }*/
        if (r + 1 != i)
          getch();
        r = i;
    ...
    strcpy(a, "I am Grimsley....The master of Dark type Pokemon, you have come "
              "far....but your journey ends here!");
    cout << "Grimsley : ";
    len = strlen(a);
    for (int i = 0; i < len; i++) {
      delay(30);
    ...
    sprintf(a, "Champion Steven: Hmmm....strong with you the will is.");
    int tht = textheight(a);
...
int health_bar(struct poke c, struct poke d, struct atk lst[4]) {
  ...
  hpp[0] = ((c.hp[0] / c.hp[1]) * 200);
  hpp[1] = ((d.hp[0] / d.hp[1]) * 200);
  ...
  do {
    ...
    sprintf(msg, "%s: %.2f", d.name, d.hp[0]);
    setcolor(RED);
...
void atkf(struct poke &c, struct poke &d, struct atk m) {
  ...
  sprintf(msg, "Damage Done = %.2f", dsp);
  outtextxy((x / 2) - wd, (28 * ht), msg);
  ...
  // cout<<"Damage Done = "<<setprecision(2)<<((dmg*1)/1);
  // sprintf(msg, "Damage Done = %.3i", int(dmg));
}
...
void championship(struct poke pk_list1[], struct poke pk_list2[],
  ...
  struct atk at1[6][4], at2[6][4];
  int s1, s2, a1, a2, next, c, i;
  clrscr();

  choosep(pk1, pk_list1, pk_list2, 6, 0, 0, log1); // working on log required
  clrscr();
    ...
    for (int k = 0; k < 30; k++) {
      ...
      for (int l = 0; l < 2; l++) {
        ...
        if (a2 == 4) {
          ...
          z = 1;
          l = l + 1;
          a2 = health_bar(pk2[s2], pk1[s1], at2[s2]);
        }
        if (z == 1) {
...
lg:;
  ...
  list:;
    clrscr();
    int e, x, y, z, a, c = 0, pla, s1 = 0, s2 = 0;
    struct poke pk1[6], pk2[6];
    atk m[3][4], n[3][4];
    ...
    for (int k = 0; k < 15; k++) {
      ...
      for (int l = 0; l < 2; l++) {
        if (l == 0) {
          a = health_bar(pk1[s1], pk2[s2], m[s1]);
          a = a - 1;
          textcolor(12);
          if (pla == 2) {
            e = health_bar(pk2[s2], pk1[s1], n[s2]);
            e = e - 1;
          } else {
